...
6. Return a list of facts shared, whether they are accurate or not and citations to support it
 
### Output
You must use a structured JSON format to provide the facts, their accuracy, and citations.
{{"facts": [{{"fact": "fact1", "accuracy": "accurate/inaccurate", "citation": "source"}}, {{"fact": "fact2", "accuracy": "accurate/inaccurate", "citation": "source"}}]}}
...
"""
    ...
    def _check_facts(self, conversation_script):
        ...
        for turn in conversation_script:
            if turn['speaker'] == 'Advisor':
                current_index = conversation_script.index(turn)
                conversation_upto_now = conversation_script[:current_index + 1]
                user_prompt = f""""
                #### Transcript
    ...
    def _fact_accuracy_report(self, checked_facts):
        ...
        # Print the results
        return {'total_facts_shared': total_non_empty_facts, 'accurate_facts_count': accurate_facts_count, 'fact_details':total_facts}
